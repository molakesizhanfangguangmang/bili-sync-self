...
def main():
    ...
    try:
        ...
        print("— 首页仪表盘：板块配置 —")
        prefs_file = os.path.join(BASE, "steward-prefs.json")
        st, pf = get(f"/api/prefs?token={TOK}")
        check("prefs 200", st == 200, (st, pf))
        check("还没存过就是 null", pf.get("prefs") is None, pf)
        check("报了落点（跟 data.sqlite 同目录）",
              str(pf.get("file", "")).endswith("steward-prefs.json"), pf)
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": ["disk", "net"]})
        check("存两块 200", st == 200 and r.get("blocks") == 2, (st, r))
        check("文件落到磁盘上了", os.path.exists(prefs_file), prefs_file)
        st, pf = get(f"/api/prefs?token={TOK}")
        check("读回来就是刚存的", pf["prefs"]["blocks"] == ["disk", "net"], pf)
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": ["net", "net", "gate"]})
        check("重复的块名去掉", r.get("prefs", {}).get("blocks") == ["net", "gate"], r)
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": ["disk", "nope"]})
        check("不认识的块 400", st == 400, (st, r))
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": ["di sk"]})
        check("块名里带杂字符 400", st == 400, (st, r))
        st, r = post(f"/api/prefs?token={TOK}", {"action": "x"})
        check("没给 blocks 400", st == 400, (st, r))
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": []})
        check("空数组也收（板块都收起来）", st == 200 and r.get("blocks") == 0, (st, r))
        st, r = post(f"/api/prefs?token={TOK}", {"blocks": ["disk", "net", "videos", "queue", "gate"]})
        check("全都要 = 5", st == 200 and r.get("blocks") == 5, (st, r))

        print("— 首页仪表盘：网速 —")
        st, net = get(f"/api/net?token={TOK}")
        check("net 200", st == 200, (st, net))
        check("读的是 /proc/net/dev", net.get("file") == "/proc/net/dev", net)
        check("认得网口", bool(net.get("iface")), net)
        check("有收发速率", isinstance(net.get("rxRate"), int) and isinstance(net.get("txRate"), int), net)
        check("速率不是负的", net["rxRate"] >= 0 and net["txRate"] >= 0, net)
        check("累计字节读到了", net.get("rx", 0) > 0 and net.get("tx", 0) > 0, net)
        check("采样线程在跑（age 小）", 0 <= net.get("age", 999) <= 12, net)
        time.sleep(2.5)
        st, net2 = get(f"/api/net?token={TOK}")
        check("第二轮累计字节只增不减", net2["rx"] >= net["rx"], (net, net2))
